...
export function SignInForm({ defaultNext }: { defaultNext: string }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const target = role ? getPostLoginPath(role) : defaultNext;
    router.push(target);
    router.refresh();
  }
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-col">
      <div className="flex flex-col items-center text-center">
        ...
        <h1 className="mt-6 text-xl font-bold uppercase tracking-[0.2em] text-foreground">Login</h1>
        <p className="mt-2 text-sm text-muted-foreground">Accède à ton espace sécurisé.</p>
      </div>
      ...
      <form className="mt-10 grid gap-7" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col-reverse gap-4 sm:flex-row sm:items-center sm:justify-between">
          <Link
            href="/reset-password"
            className="text-center text-sm font-medium text-primary underline-offset-4 transition-colors hover:text-accent sm:text-left"
          >
            Mot de passe oublié ?
          </Link>
          <Button
            type="submit"
            className={cn(
              "h-11 rounded-full border-0 bg-gradient-to-r from-primary via-primary to-accent px-10 text-xs font-semibold uppercase tracking-[0.22em] text-primary-foreground antialiased",
